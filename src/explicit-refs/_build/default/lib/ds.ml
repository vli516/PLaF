...
type exp_val =
  ...
  (* --- Part I addition: records with mutable fields --- *)
...
(* Monadic helper functions (original base) *)
...
let sequence (cs: ('a ea_result) list) : ('a list) ea_result =
  let mcons p q = p >>= fun x -> q >>= fun y -> return (x :: y) in
  List.fold_right mcons cs (return [])

let mapM (f:'a -> 'b ea_result) (vs:'a list) : ('b list) ea_result =
  sequence (List.map f vs)

(* Operations on environments (original) *)
let empty_env : unit -> env ea_result =
  fun () -> return EmptyEnv

let extend_env : string -> exp_val -> env ea_result =
  fun id v -> fun env -> Ok (ExtendEnv (id, v, env))

let extend_env_rec : string -> string -> expr -> env ea_result =
  fun id par body -> fun env -> Ok (ExtendEnvRec (id, par, body, env))
...
let rec apply_env : string -> exp_val ea_result =
  fun id env ->
    ...
    | ExtendEnv (v, ev, tail) ->
        if id = v then Ok ev else apply_env id tail
    | ExtendEnvRec (v, par, body, tail) ->
        if id = v then Ok (ProcVal (par, body, env))
...
(* Original string_of_expval, updated to handle records *)
let rec string_of_expval = function
  | NumVal n -> "NumVal " ^ string_of_int n
  | BoolVal b -> "BoolVal " ^ string_of_bool b
  | ProcVal (id, body, env) ->
      "ProcVal (" ^ id ^ ", " ^ string_of_expr body ^ ", " ^ string_of_env' [] env ^ ")"
  | PairVal (v1, v2) ->
      "PairVal(" ^ string_of_expval v1 ^ ", " ^ string_of_expval v2 ^ ")"
  | TupleVal evs ->
      "Tuple (" ^ String.concat ", " (List.map string_of_expval evs) ^ ")"
  | UnitVal -> "UnitVal"
  | RefVal i -> "RefVal (" ^ string_of_int i ^ ")"
  | RecordVal fs ->
      "RecordVal(" ^ String.concat ", " (List.map (fun (n, (b, ev)) ->
          n ^ (if b then "<=" else "=") ^ string_of_expval ev
      ) fs) ^ ")"
and string_of_env' ac = function
  ...
  | ExtendEnvRec (id, param, body, env) ->
      string_of_env' ((id ^ ":=Rec(" ^ param ^ ", " ^ string_of_expr body ^ ")") :: ac) env
